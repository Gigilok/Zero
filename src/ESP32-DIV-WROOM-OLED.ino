#include <Arduino.h>
#include "PCF8574Adapter.h"   /* replaces <PCF8574.h> */
#include "DisplayAdapter.h"   /* replaces <TFT_eSPI.h> */
#include <Wire.h>
...
void setBrightness(uint8_t value) {
  ...
  // No backlight on the OLED board — value is silently ignored.
  (void)value;
#else
...
#if defined(BOARD_ESP32_WROOM_OLED)
  // Initialize I2C on the user-specified pins: SDA=21, SCL=22.
  Wire.begin(21, 22);
  Wire.setClock(400000);
  Serial.println("[boot] I2C ready (SDA=21, SCL=22 @ 400kHz)");

  // Initialize the SSD1306 OLED via the DisplayAdapter shim. The shim handles
  // tft.init() / setRotation() / fillScreen() internally; no backlight pin.
  tft.init();
  tft.setRotation(TFT_ROTATION);
  Serial.println("[boot] OLED 0.96\" ready (white background)");
#else
  tft.init();
  tft.setRotation(TFT_ROTATION);
...
#if defined(BOARD_ESP32_WROOM_OLED)
  // LittleFS-backed settings load directly (no SD dependency to defer).
  settingsLoad();
  applyThemeToPalette(settings().theme);
...
#if defined(BOARD_ESP32_WROOM_OLED) || BOARD_HAS_ESP32S3
  // Background scanners + status bar task already started above.
#else
