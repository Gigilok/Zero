...
#if defined(BOARD_ESP32_WROOM_OLED)
  // No SD card on this board — mount LittleFS on internal flash instead.
  // The SDFlashAdapter shim handles all subsequent SD.* calls.
  if (SD.begin(SD_CS, SPI, 4000000)) {
    s_sdFsMounted = true;
    sdCardPresent = true;
    Serial.println(F("[sd] LittleFS mounted (flash storage ready)"));
  } else {
    s_sdFsMounted = false;
    sdCardPresent = false;
    Serial.println(F("[sd] LittleFS mount failed"));
  }
  updateSdCardStatus();
  return;
#endif
...
#if defined(BOARD_ESP32_WROOM_OLED)
  // On this board "SD" is actually LittleFS on flash — always available after
  // initSDCard() mounted it. If for some reason the mount was lost, try again.
  if (s_sdFsMounted) {
    return true;
  }
  if (SD.begin(SD_CS, SPI, 4000000)) {
