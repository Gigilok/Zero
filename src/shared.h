...
#if (defined(BOARD_ESP32_DIV_V2) + defined(BOARD_CYD) + defined(BOARD_ESP32_DIV_V1) + defined(BOARD_ESP32_WROOM_OLED)) > 1
#error "Select only one board: BOARD_ESP32_DIV_V2, BOARD_ESP32_DIV_V1, BOARD_CYD, or BOARD_ESP32_WROOM_OLED"
#endif

#if defined(BOARD_ESP32_WROOM_OLED)
#ifndef ESP32DIV_BOARD_NAME
#define ESP32DIV_BOARD_NAME "ESP32 WROOM 30p + OLED 0.96\""
#endif
#ifndef HAS_PCF8574_BUTTONS
#define HAS_PCF8574_BUTTONS 1   /* shim — buttons wired directly to GPIO */
#endif
#ifndef BOARD_HAS_ESP32S3
#define BOARD_HAS_ESP32S3 0     /* classic ESP32 WROOM, not S3 */
#endif
...
#endif
#else
#error "Unknown board: define BOARD_ESP32_DIV_V2, BOARD_ESP32_DIV_V1, BOARD_CYD, or BOARD_ESP32_WROOM_OLED"
#endif
...
/* ── Button pin map ──
 * Original boards route the buttons through a PCF8574 I2C expander; the index
 * space below (BTN_UP/DOWN/LEFT/RIGHT/SELECT) selects which PCF8574 pin to read.
 * On BOARD_ESP32_WROOM_OLED the PCF8574 shim maps these indices to physical GPIOs:
 *   BTN_UP     -> GPIO 5   (cinza)
 *   BTN_DOWN   -> GPIO 27  (vermelho)
 *   BTN_SELECT -> GPIO 32  (marrom)
 *   BTN_LEFT   -> GPIO 33  (preto)   -- the optional 4th button
 *   BTN_RIGHT  -> unused (no 5th button on the user's hardware)
 */
#if defined(BOARD_ESP32_WROOM_OLED)
#define BTN_UP       0
#define BTN_DOWN     1
...
/* Buzzer — ESP32 WROOM 30-pin board: buzzer on GPIO 4. */
#ifndef BUZZER_PIN
#if defined(BOARD_ESP32_WROOM_OLED)
#define BUZZER_PIN 4
#else
...
/* Backlight / PWM — the OLED has no backlight; the PWM channel is unused. */
...
/* SD Card
 * On BOARD_ESP32_WROOM_OLED there is no SD card — storage uses LittleFS
 * on the ESP32's internal flash via the SDFlashAdapter shim. We still expose
 * the SD_* pins so existing code that references them compiles, but the SD_*
 * values are unused because the shim ignores the SPI bus entirely.
 *
 * IMPORTANT: SD_CS, SD_MOSI, SD_MISO, SD_SCLK must NOT collide with any
 * functional GPIO on this board (buttons, radio CS, buzzer, etc.). We pick
 * GPIO 15 for SD_CS (an unused output-capable pin) and the shared SPI lines
 * (18/19/23) for SD_MISO/MOSI/SCLK — the SDFlashAdapter shim ignores these
 * assignments, but the project's sdSpiInit() will call SPI.begin() with
 * them and we don't want that to re-configure a button pin as OUTPUT.
 */
#ifndef SD_CS
#if defined(BOARD_ESP32_WROOM_OLED)
#define SD_CS    15    /* unused GPIO — kept for source compatibility; shim ignores it */
#elif defined(BOARD_CYD)
#define SD_CS    5
#elif defined(BOARD_ESP32_DIV_V1)
#define SD_CS    5
...
#endif
#endif
#if !defined(BOARD_CYD) && !defined(BOARD_ESP32_DIV_V1) && !defined(BOARD_ESP32_WROOM_OLED) && !defined(SD_CD)
#define SD_CD    38
#endif
#ifndef SD_CS_PIN
#if defined(BOARD_ESP32_WROOM_OLED)
#define SD_CS_PIN 15    /* unused — same as SD_CS; shim ignores */
#else
#define SD_CS_PIN 5
#endif
#endif

/* PN532 RFID/NFC (SPI) — optional. On BOARD_ESP32_WROOM_OLED we share the SPI
 * bus with CC1101/nRF24 (SCK=18, MISO=19, MOSI=23) and pick GPIO 17 for SS
 * (an unused output-capable pin). Override in BoardConfig.h if needed. */
#ifndef PN532_SCK
#if defined(BOARD_ESP32_WROOM_OLED)
#define PN532_SCK  18
#elif defined(BOARD_CYD)
...
#ifndef PN532_SS
#if defined(BOARD_ESP32_WROOM_OLED)
#define PN532_SS   17   /* unused GPIO on this board */
#elif defined(BOARD_CYD)
#define PN532_SS   25
#else
#define PN532_SS   5
...
#ifndef RX_PIN
#if defined(BOARD_ESP32_WROOM_OLED)
#define RX_PIN 16   /* unused GPIO on this board */
#elif defined(BOARD_CYD)
#define RX_PIN 35
#elif defined(BOARD_ESP32_DIV_V1)
#define RX_PIN 16
...
#ifndef TX_PIN
#if defined(BOARD_ESP32_WROOM_OLED)
#define TX_PIN 17   /* unused GPIO on this board */
#elif defined(BOARD_CYD)
#define TX_PIN 22
#elif defined(BOARD_ESP32_DIV_V1)
#define TX_PIN 26
...
#ifndef GPS_UART_RX
#if defined(BOARD_ESP32_WROOM_OLED)
#define GPS_UART_RX 16   /* unused GPIO on this board */
#elif defined(BOARD_CYD)
...
#ifndef GPS_UART_TX
#if defined(BOARD_ESP32_WROOM_OLED)
#define GPS_UART_TX 17   /* unused GPIO on this board */
#elif defined(BOARD_CYD)
...
/* CC1101 (Sub-GHz) — user wiring for BOARD_ESP32_WROOM_OLED:
 *   1 GND  = GND        (cinza)
 *   2 3V3  = 3.3V       (branco)
 *   3 GDO0 = GPIO 12    (azul)
 *   4 CSN  = GPIO 14    (roxo)
 *   5 SCK  = GPIO 18    (amarelo)
 *   6 MOSI = GPIO 23    (verde)
 *   7 MISO = GPIO 19    (vermelho)
 *   8 GDO2 = GPIO 13    (laranja)
 */
#ifndef CC1101_SCK
#if defined(BOARD_ESP32_WROOM_OLED)
...
#if defined(BOARD_ESP32_WROOM_OLED)
#define SUBGHZ_RX_PIN 13   /* CC1101 GDO2 */
#elif defined(BOARD_CYD)
...
#if defined(BOARD_ESP32_WROOM_OLED)
#define SUBGHZ_TX_PIN 12   /* CC1101 GDO0 */
#elif defined(BOARD_CYD)
...
/* NRF24 — user wiring for BOARD_ESP32_WROOM_OLED:
 *   1 CE  = GPIO 26   (amarelo)
 *   2 CSN = GPIO 25   (azul)
 *   3 SCK = GPIO 18   (verde)    -- shared SPI bus with CC1101
 *   4 MO  = GPIO 23   (roxo)
 *   5 MI  = GPIO 19   (cinza)
 *   RO    = sem conexão (no IRQ)
 */
#ifndef CE_PIN_1
#if defined(BOARD_ESP32_WROOM_OLED)
#define CE_PIN_1  26
...
#ifndef CE_PIN_2
#if defined(BOARD_ESP32_WROOM_OLED)
#define CE_PIN_2  26   /* same radio — single nRF24 on this board */
#elif defined(BOARD_CYD)
#define CE_PIN_2  22
#elif defined(BOARD_ESP32_DIV_V1)
#define CE_PIN_2  26
 ...
 *
 * On BOARD_ESP32_WROOM_OLED we move IR_RX to GPIO 35 (input-only, perfect for
 * an IR receiver diode) and IR_TX to GPIO 2 (unused output-capable pin that
 * can PWM an IR LED). Neither conflicts with the user-specified wiring.
 */
#ifndef IR_RX_PIN
#if defined(BOARD_ESP32_WROOM_OLED)
#define IR_RX_PIN 35   /* input-only pin — safe for IR receiver */
#elif defined(BOARD_CYD)
#define IR_RX_PIN 35
#elif defined(BOARD_ESP32_DIV_V1)
#define IR_RX_PIN 4
...
#ifndef IR_TX_PIN
#if defined(BOARD_ESP32_WROOM_OLED)
#define IR_TX_PIN 2    /* unused GPIO — safe for IR LED output */
#elif defined(BOARD_CYD)
#define IR_TX_PIN 4
#elif defined(BOARD_ESP32_DIV_V1)
#define IR_TX_PIN 5
...
#if defined(BOARD_ESP32_WROOM_OLED)
#define BATTERY_ADC_PIN 34   /* user-specified battery divider tap */
#elif defined(BOARD_ESP32_DIV_V1)
