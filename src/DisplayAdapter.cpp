...
// NOTE: TFT_eSPI::scrollUp is now defined INLINE in DisplayAdapter.h
// (moved there to resolve a linker "undefined reference" error from
// utils.cpp.o — the out-of-line definition below was being dropped from
// the link for an unknown reason; inlining guarantees the symbol is
// available in every translation unit that calls it).
...
// 4-arg drawChar(unsigned char, int16_t, int16_t, uint8_t) is now defined
// INLINE in DisplayAdapter.h (moved there to resolve a linker "undefined
// reference" error from utils.cpp.o — the out-of-line definition below was
// being dropped from the link for an unknown reason; inlining guarantees the
// symbol is available in every translation unit that calls it).
